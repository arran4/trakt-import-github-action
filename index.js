...
var transform = function(original_options = {}) {
  ...
  return {
    ...
    __isDelimiter: function(buf, pos, chr) {
      ...
      loop1:
        for (let i2 = 0; i2 < delimiter.length; i2++) {
...
var parse = function() {
  ...
  for (const i2 in arguments) {
    const argument = arguments[i2];
    ...
    if (data === void 0 && (typeof argument === "string" || Buffer.isBuffer(argument))) {
      ...
      throw new CsvError("CSV_INVALID_ARGUMENT", [
        "Invalid argument:",
        `got ${JSON.stringify(argument)} at index ${i2}`
      ], options || {});
    }
  }
...
function getAccessToken(redirectUrl, clientId, clientSecret, refreshToken) {
  return __async(this, null, function* () {
    ...
    let fetchResult = yield fetch("https://trakt.tv/oauth/token", {
      method: "POST",
    ...
    let refreshResult = yield fetchResult.json();
    let accessToken = refreshResult == null ? void 0 : refreshResult.access_token;
    if (!accessToken) {
...
function post(url, clientId, accessToken, data) {
  return __async(this, null, function* () {
    if (lastPost != null) {
      yield new Promise((e2) => setTimeout(e2, lastPost.getDate() - new Date().getDate()));
    }
    let fetchResult = yield fetch(url, {
      method: "POST",
      body: JSON.stringify(data),
...
function SyncHistory(_0, _1) {
  return __async(this, arguments, function* (accessToken, clientId, episodes = [], seasons = [], shows = [], movies = []) {
    ...
    let fetchResult = yield post(void 0, clientId, accessToken, data);
...
function run() {
  return __async(this, null, function* () {
    ...
    const fileContent = fs2.readFileSync(netflixFilePath, { encoding: "utf-8" });
    let watchList = yield new Promise((ret, err) => parse(fileContent, {
      delimiter: ",",
      columns: headers
    }, (ca, cb) => {
      if (ca) {
    ...
    yield SyncHistory(accessToken, clientId);
    return "Done";
  });
}
